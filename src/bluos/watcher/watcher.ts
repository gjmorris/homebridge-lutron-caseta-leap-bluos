...
const kitchenVolumeRatio = 2.5; // living volume is divided by this to set kitchen volume
 ...
 */
async function updateLivingKitchenSyncState(): Promise<void> {
  ...
  if (livingStatus.isTv) {
    ...
    if (!livingStatus.slaves?.some(slave => slave.id === players.kitchen.ip)) {
      await controller.volumeLevel(players.kitchen, Math.round(livingStatus.volume / kitchenVolumeRatio));
...
for (;;) {
  try {
    await updateLivingKitchenSyncState();
